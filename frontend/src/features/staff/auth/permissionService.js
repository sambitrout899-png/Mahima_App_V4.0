  ...
  _rolesPromise = (async () => {
    try {
      ...
      for (const r of roles) {
        ...
        // normalized (lowercase) page keys for case-insensitive matching
...
/**
 * getToken
 * Returns token from stored user object or from common localStorage keys.
 */
...
export async function canAccessPage(pageKey) {
  ...
  // Admin bypass — case-insensitive page keys
  if (isAdminRole(role)) {
    // Admin always allowed for these admin UI pages (lower-case)
    const adminAlwaysPages = ["pages", "roles"];
    if (adminAlwaysPages.includes(String(pageKey).toLowerCase())) return true;
    // uncomment the next line to allow admin everywhere:
    // return true;
  }
...
// default export for consumers that prefer it
export default {
  fetchRolesMap,
  getCurrentUser,
  getToken,
