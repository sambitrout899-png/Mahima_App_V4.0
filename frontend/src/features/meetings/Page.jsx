...
export default function Page() {
  ...
  async function save() {
    try {
      if (editing) {
        ...
        await meetingsApi.update(id, form);
  ...
  async function doDelete(id) {
    ...
    try { await meetingsApi.remove(id); await load(); alert('Deleted'); } catch(e) { console.error(e); alert('Delete failed: ' + (e && e.message? e.message: e)); }
  }
  ...
  return (
    <div>
      <div className='flex justify-between items-center mb-4'>
        <h2 className='text-xl font-semibold capitalize'>{'meetings'}</h2>
        ...
        <Modal title={editing ? 'Edit Record' : 'Add Record'} onClose={() => setOpen(false)}>
          <div className='space-y-3'>
            {(DetectFields(editing || items[0] || { id:'', name:'' })).map(key => {
              ...
              const readonly = (('meetings' === 'users') && (key.toLowerCase().includes('joindate') || isId));
              return (
                <div key={key}>
