...
export default function TeamsPage() {
  ...
  const fetchTeams = async (page = 1, limit = 50, q = "") => {
    ...
    try {
      ...
      if (!resp.ok) {
        const txt = await resp.text().catch(() => "");
        throw new Error(`GET /teams failed ${resp.status} ${txt || resp.statusText}`);
      }
  ...
  const deleteTeam = async (id) => {
    ...
    try {
      ...
      diag(`DELETE /teams/${id} -> success`, { status: res.status });
  ...
  const save = async (ev) => {
    ...
    try {
      ...
      setIsOpen(false);
      diag(`${editing ? "PUT" : "POST"} /teams -> success`, { payload, status: res.status });
