...
files.forEach(file => {
  ...
  // Common problematic patterns where /api/... was unquoted and became a regex:
  // 1) call(/api...
  updated = updated.replace(/call\(\s*\/api/g, "call('/api");
  updated = updated.replace(/call\(\s*\/\s*api/g, "call('/api"); // loose spacing

  // 2) fetch(/api...
  updated = updated.replace(/fetch\(\s*\/api/g, "fetch('/api");
  updated = updated.replace(/fetch\(\s*\/\s*api/g, "fetch('/api");

  // 3) direct token starts like ( /api or = /api or : /api or , /api
  ...
  // Safety check: ensure single-quote count didn't go odd (simple guard)
  ...
  if (((origSingleQuotes - newSingleQuotes) % 2) !== 0) {
    console.warn(`Skipping write for ${file} — quote-count changed oddly. Manual check needed.`);
    return;
  }
  ...
  if (updated !== text) {
    ...
    // print a short preview of first 30 lines for inspection
