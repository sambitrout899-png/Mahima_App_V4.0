...
export default function MembersPage() {
  ...
  async function loadMembers() {
    ...
    setError(null);
    try {
      const res = await fetch(apiPath(`/api/teams/${encodeURIComponent(teamId)}/members`), { method: "GET" });
  ...
  async function enrichMembersWithUserDetails(list) {
    ...
    for (let i = 0; i < need.length; i += CHUNK) {
      ...
      await Promise.allSettled(chunk.map(async (uid) => {
        try {
          const res = await fetch(apiPath(`/api/users/${encodeURIComponent(uid)}`));
          if (!res.ok) return;
          const parsed = await parseResponse(res);
  ...
  async function unsetLeaderMembership(existing) {
    ...
    try {
      const res = await fetch(apiPath(`/api/teams/${encodeURIComponent(teamId)}/members/${encodeURIComponent(existingUserId)}`), {
        method: "PUT",
      ...
      const res2 = await fetch(apiPath(`/api/teams/${encodeURIComponent(teamId)}/members`), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
  ...
  async function setLeaderForMember(member, desired) {
    ...
    try {
      ...
      const res = await fetch(apiPath(`/api/teams/${encodeURIComponent(teamId)}/members/${encodeURIComponent(uid)}`), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
  ...
  async function addMember(userToAdd = null, opts = {}) {
    ...
    try {
      ...
      const res = await fetch(apiPath(`/api/teams/${encodeURIComponent(teamId)}/members`), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
  ...
  async function removeMember(member) {
    ...
    try {
      const res = await fetch(apiPath(`/api/teams/${encodeURIComponent(teamId)}/members/${encodeURIComponent(uid)}`), { method: "DELETE" });
      const parsed = await parseResponse(res);
  ...
  async function fetchUserTeamsAndUpdate(userIdToFetch) {
    ...
    try {
      const res = await fetch(apiPath(`/api/users/${encodeURIComponent(userIdToFetch)}/teams`));
      const parsed = await parseResponse(res);
      if (!res.ok) {
