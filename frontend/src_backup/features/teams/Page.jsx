...
export default function TeamsPage() {
  ...
  const deleteTeam = async (id) => {
    ...
    } catch (err) { console.error(err); diag(`DELETE /api/teams/${id} -> error`, String(err)); alert("Delete failed: " + (err?.message || String(err))); }
  };
  const save = async (ev) => {
    ev?.preventDefault?.();
    ...
    try {
      ...
      if (editing && form.id) {
        ...
        res = await fetch(`${API_BASE}/api/teams`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      }
    ...
    } catch (err) { console.error(err); diag(`${editing ? "PUT" : "POST"} /api/teams -> error`, String(err)); alert("Save failed: " + (err?.message || String(err))); }
  ...
  const runDiagnostics = async () => {
    ...
    try {
      ...
      diag("GET /api/teams", { status: g.status, ok: g.ok, body });
