...
export default function RolesPage() {
  ...
  const openEdit = (r) => { setForm({ id: r.id, name: r.name || "", description: r.description || "", pages: r.pages || [] }); setPagesFilter(""); setShow(true); };
  ...
  const selectAllFiltered = () => {
    const list = filteredPages.map(p => p.key);
  ...
  const save = async (e) => {
    ...
    setSaving(true);
    try {
      const payload = { name: form.name.trim(), description: (form.description || "").trim() || null, pages: form.pages || [] };
      const url = form.id ? `${API_BASE}/roles/${encodeURIComponent(form.id)}` : `${API_BASE}/roles`;
  ...
  const confirmDelete = async (id) => {
    ...
    try {
      const resp = await fetch(`${API_BASE}/roles/${encodeURIComponent(id)}`, { method: "DELETE" });
  ...
  const Styles = (
    <style>{`
      ...
      .roles-header{ position:sticky; top:8px; z-index:40; display:flex; gap:10px; align-items:flex-start; padding:14px; border-radius: var(--radius); background: linear-gradient(180deg,rgba(255,255,255,0.9),rgba(255,250,240,0.95)); box-shadow: var(--shadow); border:1px solid rgba(0,0,0,0.04); margin-bottom:10px; }
