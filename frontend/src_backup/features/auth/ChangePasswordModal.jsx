...
export default function ChangePasswordModal({ open, onClose, presetUsername }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const resp = await fetch(`${import.meta.env.VITE_API_BASE || '/api'}/auth/change-password`, {
        method: "POST",
        headers: {
      ...
      if (resp.ok) {
        ...
        if (json?.token) {
          ...
          setMessage("Password updated â€” you're still signed in.");
        } else {
    ...
    <div style={backdropStyle} onMouseDown={(e) => { if (e.target === e.currentTarget) onClose?.(); }}>
      <form onSubmit={submit} style={panelStyle} role="dialog" aria-modal="true" aria-label="Change password">
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <div style={{ width: 52, height: 52, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(180deg,#fff6e3,#fff1d6)", fontSize: 22 }}>ðŸ”’</div>
          <div>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 6 }}>
          ...
          <button type="submit" disabled={loading || success} style={{ display: "inline-flex", gap: 8, alignItems: "center", background: "linear-gradient(90deg,#f1c232,#f6d980)", border: "none", padding: "8px 14px", borderRadius: 8, fontWeight: 800 }}>{loading ? "Savingâ€¦" : success ? "Saved âœ“" : "Save"}<span aria-hidden>{loading ? "â³" : "ðŸ’¾"}</span></button>
        </div>
      </form>
    </div>
  );
